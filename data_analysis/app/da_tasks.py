...
def pattern_finder(token: str, threshold=0.5):
    data = get_cost_info(token)
    p = PatternFinder(data)
    # 날짜는 수정이 가능함 원하는 날짜가 들어오게 만들면 될 듯
    start_date = data["ResultsByTime"][0]["TimePeriod"]["Start"]
    end_date = data["ResultsByTime"][-1]["TimePeriod"]["End"]
    result = p.search(start_date, end_date, threshold=threshold)
